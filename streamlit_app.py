...
def ensure_headers(sheet):
    ...
    current_headers = [h.strip() for h in current_headers if h.strip() != ""]

    # Auto-correct: replace headers if missing or incorrect
    if current_headers != EXPECTED_HEADERS:
        sheet.delete_row(1) if len(current_headers) > 0 else None
...
def save_to_google_sheet(data):
    ...
    if len(data) != len(headers):
        raise ValueError(f"Expected {len(headers)} columns, but got {len(data)} values.")
    ...
    **University of Malaya**
    ...
    if st.button("Submit Survey"):
        ...
        try:
            ...
            st.error(f"❌ Submission error: {ve}")
